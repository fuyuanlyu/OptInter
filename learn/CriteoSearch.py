...
parser.add_argument('--tau', type=float, default=5e-5, help='Tau value for gumbel softmax')
...
def main():
    ...
    model = DNN_cart_search(cont_field, cate_field, comb_field, 
            ...
            selected_pairs=selected_pairs, tau=args.tau,
    ...
    if hasattr(model, 'convert_layer'):
        unregularized_param += list(model.convert_layer.parameters())
    ...
    for x, y in get_data(dataset_folder, name='test', bsize=args.bsize, \
        ...
        for feature_name in ['feat_conts', 'feat_cates', 'feat_combs']:
            if feature_name in x.keys():
                ...
                features.append(torch.tensor((), dtype=my_dtype, device=device).new_ones((batch_size, -1)))
